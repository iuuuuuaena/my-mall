  ...
  <div id="main">
    <div class="content page-order-payment">
      ...
      <div class="confirm-detail">
        ...
        <p class="info-detail">姓名：{{orderInfo.receiveInfo.user_nickname}}</p>
      ...
      <div class="confirm-goods-table">
        <div class="cart-items clear" v-for="(item,index) in orderInfo.goodsData" :key="index">
          ...
          <div class="price fn-right">¥ {{item.goods_price}}.00</div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    payNowHandle(id) {
      ...
      setTimeout(() => {
        div.innerHTML = this.$store.state.formPage;
        console.log("这个为");
        console.log(this.$store.state.formPage);
        console.log(this.$store.state.orderData);
        let routeData = this.$router.resolve({
          path: "pay",
          query: {
            htmls: this.$store.state.formPage
          }
        });
        // 新窗口打开
        window.open(routeData.href, "_blank");
      }, 2000);
      // 如果一分钟没有付款，就路由到支付成功界面
      // setTimeout(() => {
      //   this.$router.push({ name: "Account" });
      // }, 60000);
      // alert("1234567"); // data就是接口返回的form 表单字符串

      // 2、新建pay.vue页面
    }
    //   this.$store.status.order.isPay = true;
    //   if (result != null) {
    //     // console.log(result.data);
