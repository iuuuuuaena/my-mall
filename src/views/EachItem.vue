            ...
            <div class="thumbnail">
              <!-- <ul>
                <li
                  :class="{'on':index==0}"
                  :key="index"
                  v-for="(img,index) in itemList.goods_image"
              >-->
              <!-- <img :src="img+'?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'" /> -->
              <img :src="'http://121.89.208.41:8080'+itemList.goods_image" />
        ...
        <div class="banner">
          <div class="sku-custom-title">
            <div class="params-price">
              <span>
                <em>¥</em>
                <i>{{itemList.goods_price}}</i>
              </span>
            </div>
            <div class="params-info">
              <h4>{{itemList.goods_name}}</h4>
              <h6>{{itemList.user_nickname}}</h6>
            </div>
          </div>
            ...
            <div class="sku-dynamic-params clear">
              ...
              <!-- <ul class="params-colors">
                <li
                  ...
                  v-for="(color,index) in itemList.sku_list"
                  :key="index"
                >
            ...
            <div class="sku-dynamic-params clear">
              ...
              <div class="params-detail clear">
                <div class="item-num js-select-quantity">
                  ...
                  <span
                    class="up"
                    :class="{'up-disabled':count>=itemList.is_deal}"
                    @click="addCount"
                  >+</span>
                </div>
              </div>
          ...
          <div class="sku-status">
            <div class="cart-operation-wrapper clearfix">
              ...
              <span class="gray-title-btn" @click="checkOutHandle">
                <a>现在购买</a>
              </span>
            </div>
          </div>
...
<script>
import itemsData from "@/lib/newGoodsData.js";
...
export default {
  ...
  computed: {
    // //  商品信息
    // itemList() {
    //   let itemList = this.itemsList.filter(item => {
    //     // 商品信息从query中获取
    //     return item.goods_id == this.$route.query.itemId;
    //   })[0];
    //   return itemList;
    // },
    // 提示框出现的函数
  ...
  methods: {
    ...
    addCount() {
      // 如果选择的数量大于商品的总数的话！
      this.count++;
      if (this.count > this.itemList.is_deal) {
        this.count = this.itemList.is_deal;
    ...
    checkOutHandle() {
      ...
      itemList.count = this.count;
      let provisionalOrder = {
        totlePrice: this.count * this.itemList.goods_price,
    ...
    addCarPanelHandle() {
      let data = [this.itemList, this.count];
  ...
  // 页面加载前
  mounted(){
    // console.log("当前页面商品的id为");
    
    // console.log(this.$route.query.itemId);
    var goods_id = this.$route.query.itemId
    // 调用查询goodsid的方法
    this.getGoodsInfoById(goods_id)
